...
function createGetTablesInGroupParams(accessToken){
    var authHeader = getAuthHeader(accessToken);
    var headers = {
        'Authorization': authHeader,       
    };
    ...
    var url = config.apiUrl + 'v1.0/myorg/groups/' + config.workspaceId + '/datasets/' + config.datasetId + '/tables';
...
async function sendGetTablesInGroupAsync(url, options){
    let promise = () => { return new Promise(
        (resolve, reject) => {
            request(url, options,function (error, response, body){
                ...
                if(body == ""){
                    console.log('error: no dataset with id: ' + config.datasetId + " in group with id: " + config.workspaceId);
                    reject('error: no dataset with id: ' + config.datasetId + " in group with id: " + config.workspaceId);
                }
                try{
                    getReportRes = JSON.parse(body)
					console.log(getReportRes);
                    if(config.reportId){
                        console.log("Returned dataset name: " + getReportRes.name);
                        console.log("Returned dataset Id: " + getReportRes.id);
                        resolve(getReportRes);
                    } else {
                        if(getReportRes.value.length == 0) {
                            console.log('No datasets in the given workspace');
                            reject('error: No datasets in the given workspace');
                        } else {
                            console.log("Returned datasets name: " + getReportRes.value[0].name);
                            console.log("Returned datasets Id: " + getReportRes.value[0].id);
                            resolve(getReportRes.value[0]); 
                        }
                    }
                } catch(e){}
